...
class CaddyTest < Minitest::Test
  ...
  def test_sites_use_matchers_and_split_guarded_and_open_hosts
    ...
    caddy.write_site(KEY, "pkliinp6.proj.example.com", 4001, "pw")
    site = File.read(caddy.site_path(KEY))
    assert_equal File.join(@config.sites_dir, "proj", "#{KEY}.caddy"), caddy.site_path(KEY)
    assert_includes site, "@proj__feature__4001_guarded host pkliinp6.proj.example.com app-pkliinp6.proj.example.com"
    assert_includes site, "basic_auth {\n\t\tdev HASH"
    assert_includes site, "@proj__feature__4001_open host mcp-pkliinp6.proj.example.com"
  ...
  def test_without_password_everything_is_open_on_the_same_hosts
    ...
    caddy.write_site(KEY, "pkliinp6.proj.example.com", 4001, nil)
    site = File.read(caddy.site_path(KEY))
  ...
  def test_wildcard_site_covers_bare_and_folded_subdomain_hosts
    ...
    assert_includes wildcard, "import #{File.join(@config.sites_dir, 'proj')}/*.caddy"

    site = File.read(caddy.site_path(KEY))
    ["pkliinp6.proj.example.com", "app-pkliinp6.proj.example.com", "mcp-pkliinp6.proj.example.com"].each do |host|
      assert_includes site, host
      # Every served host sits one label under *.proj.example.com, so the
      # wildcard certificate covers it.
      assert_match(/\A[a-z0-9-]+\.proj\.example\.com\z/, host)
    end
  end
  ...
  def test_base_domain_change_is_refused
    ...
    assert_raises(DevEnv::Error) { changed.ensure_certificate_configuration! }
  end
